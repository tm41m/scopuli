...
, transform_1 as (
    ...
    left join {{ source('aethervest', 'product_listings_history') }} as plh
        on
            acd.val > plh.effective_from
            and acd.val <= coalesce(plh.effective_to, '9999-01-01'::timestamp)
    inner join {{ source('aethervest', 'stores') }} as s
...
select
    *
    , md5(
        concat_ws('|', calendar_date, coalesce(region_code, ''), product_id)
    ) as md5_key
from transform_1
