...
with transform_1 as (
    select
        ...
        , case
            when
                substring("Products", '(\d*\.?\d+)') is not null
                then substring("Products", '(\d*\.?\d+)')::numeric(32, 2)
            else 1
        end as amount
        , case
            when substring(rtrim("Products"), '\s*(\S+)$') = 'grams' then 'g'
            when
                substring(rtrim("Products"), '\s*(\S+)$') in (
                    'kilogram', 'kilograms'
                )
                then 'kg'
            when
                substring(rtrim("Products"), '\s*(\S+)$') in ('millilitres')
                then 'mL'
            when
                substring(rtrim("Products"), '\s*(\S+)$') in ('litres', 'litre')
                then 'L'
            when
                rtrim(substring(rtrim("Products"), '\s*(\S+)$'), ')') in (
                    'unit', 'bags', 'dozen'
                )
